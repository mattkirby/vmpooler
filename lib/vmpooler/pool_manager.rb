module Vmpooler
  class PoolManager
    ...
    def cleanup_threads(threads)
      # Remove dead threads from the threads object provided
      threads.each do |k,v|
        threads.delete(k) unless v.status
      end
    end
    ...
    def _check_pool(pool, provider)
      ...
      $logger.log('s', "[ ] [#{pool['name']}] Finished check_pool, freeing up slot")
